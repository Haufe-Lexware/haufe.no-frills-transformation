...
namespace NoFrillsTransformation.Plugins.Json
{
    ...
    public class JsonWriterFactory : ITargetWriterFactory
    {
        ...
        {
            context.Logger.Info("JsonWriterFactory: Creating a JsonWriter.");
            return new JsonWriter(context, target, GetFieldNames(fieldDefs), GetFieldSizes(fieldDefs), config);
        }
