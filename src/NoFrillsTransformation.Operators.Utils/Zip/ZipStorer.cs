...
namespace NoFrillsTransformation.Operators.Utils.Zip
{
    /// <summary>
    ...
    public class ZipStorer : IDisposable
    {
        ...
        public static bool CopyZipFile(ZipStorer _zip, string targetFileName)
        {
            List<ZipFileEntry> fullList = _zip.ReadCentralDir();

            string tempZipName = targetFileName;
            string tempEntryName = Path.GetTempFileName();

            try
            {
                ZipStorer tempZip = ZipStorer.Create(tempZipName, _zip.Comment);
