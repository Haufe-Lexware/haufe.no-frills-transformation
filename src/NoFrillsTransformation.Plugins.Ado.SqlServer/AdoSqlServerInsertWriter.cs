...
namespace NoFrillsTransformation.Plugins.Ado.SqlServer
{
    class AdoSqlServerInsertWriter : AdoSqlServerWriterBase
    {
        ...
        {
            Context.Logger.Info("AdoSqlServerInsertWriter initializing...");
            ...
            if (disposing)
            {
                ...
                if (null != _transaction)
                {
                    if (!_finished)
                    {
                        try
                        {
                            Context.Logger.Warning("AdoSqlServerInsertWriter did not finish writing successfully; rolling back transaction!");
                            _transaction.Rollback();
                        }
                        catch (Exception e)
                        {
                            Context.Logger.Error("AdoSqlServerInsertWriter: An exception occurred while rolling back the write transaction: " + e.Message);
                        }
                    }
